...
public class Player {
    ...
    public void initSets(Loader loader) {
        int i = sets.length;
        dataSets = new UnitData[i];
        for (i --; i >= 0; i --) {
            dataSets[i] = (UnitData) loader.engine.getObj(sets[i]);
        }
        sets = null;
    }

    public void jsonInit(Layer layers[], Loader loader) {
        this.loader = loader;
    ...
    public void writePlayer(DataOutputStream dos, Loader loader) throws IOException {
        dos.writeInt(unitX);
        dos.writeInt(unitY);
        dos.writeByte(layer);
        ...
        for (UnitData set : dataSets) {
            dos.writeShort(loader.engine.getIndex(set.name));
        }
        //
        dos.writeUTF(icon);
    ...
    public void readPlayer(DataInputStream dis, Loader loader) throws IOException {
        int i, n;
        unitX = dis.readInt();
        unitY = dis.readInt();
        ...
        iconAnim = loader.loadAnimation(iconPath);
        iconX = dis.readShort();
        iconY = dis.readShort();
    ...
    public void activatePlayer() {
        HeroHUD hero = loader.getObj(HeroHUD.class);
        hero.setAnimSet(loader, iconAnim);
        ...
        unit = (UnitObj) (set.createObj(loader, unitLayer, unitX, unitY, null));
        unit.owner = this;
        active = true;
    }
    ...
    public void act() {
        if (unit.getHP() == 0) {
            ...
            if (!enemy && unit.data == null) {
                loader.engine.restertAfterDead();
            }
        } else {
    ...
    public void keyReleased(int key) {
        if (canControl) {
            ...
            if (key == KeyEvent.VK_W) unit.vx = unit.vy = 0f;
        }
    }
