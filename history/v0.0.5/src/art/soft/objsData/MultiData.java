...
public class MultiData extends ObjData {
    ...
    public void loadJson(Loader loader) {
        ...
        for (i --; i >= 0; i --) {
            animsData[i] = loader.loadAnimation(anims[i]);
        }
        i = 0;
        for (FrameData frame : frames) {
            i ++;
            if (frame.next < 0) frame.next = i;
            frame.loadJson(loader);
        }
    }
    ...
    protected void readData(Loader loader, DataInputStream dis) throws IOException {
        ...
        for (i = 0; i < n; i ++) {
            animsData[i] = loader.loadAnimation(dis.readUTF());
        }
        //
        ...
        for (i = 0; i < n; i ++) {
            forms[i] = new Static();
            forms[i].readData(loader, dis);
        }
        //
        n = dis.readUnsignedByte();
        ObjData bullets[] = new ObjData[n];
        for (i = 0; i < n; i ++) {
            bullets[i] = loader.engine.getObj(dis.readUTF());
        }
        //
